# 🏚️ The 16th Room v2.1

> **MBTI Horror AI Chat Experience - 个性化恐怖AI对话体验**

## ✨ v2.1 更新 (2026-02-21)

- 📊 **30题MBTI测试** - 从20题扩展，准确度大幅提升
- 👻 **48个恐怖NPC** - 16种×3变体（心理/身体/超自然恐怖）
- 🎬 **144个场景** - 每个NPC 3个场景（normal/hard/nightmare）
- 🎲 **随机变体系统** - 每次体验不同，重玩价值提升
- 🎨 **赛博朋克主题** - 问题和场景融入赛博朋克风格

## 🎯 项目概述

The 16th Room 是一个基于 MBTI 人格类型的个性化恐怖 AI 聊天应用。通过30题心理测试确定用户的 MBTI 类型，然后随机匹配相应的"反MBTI"恐怖 NPC（3个变体之一），创造针对性的心理恐怖体验。

## ✨ 核心特性

### 🧠 MBTI 检测系统
- **30题精准MBTI测试**（E/I:8题 | S/N:8题 | T/F:7题 | J/P:7题）
- 支持所有16种人格类型识别
- 问题生活化、场景化，带有赛博朋克主题

### 👻 反MBTI 恐怖映射
- **48个恐怖NPC角色**（16种×3变体）
- 每种变体不同恐怖风格：
  - 心理恐怖（psychological）
  - 身体恐怖（body_horror）
  - 超自然恐怖（supernatural）
- 每个 NPC 专门攻击对应 MBTI 类型的弱点

### 🤖 AI 驱动对话
- 集成 MiniMax M2.5 AI 模型
- 角色化恐怖对话体验
- 动态恐怖等级升级系统

### 🎨 赛博朋克界面
- 霓虹青色主题 (#00f0ff)
- 故障闪烁动画效果
- 软浮雕面板设计
- CRT扫描线氛围
- 深度视差交互
- 响应式移动端适配

## 🎭 MBTI ↔ 恐怖NPC 映射表

| 用户类型 | 恐怖NPC | NPC特点 | 攻击策略 |
|---------|---------|---------|---------|
| INTJ | The Master Strategist | 计划控制狂 | 否定自发行为，强制系统化 |
| INTP | The Social Enforcer | 社交暴君 | 强制社交，否定独立思考 |
| ENTJ | The Passive Saboteur | 被动破坏者 | 情感操控，被动不合作 |
| ENTP | The Tradition Keeper | 传统守护者 | 用规则束缚，否定创新 |
| INFJ | The Reckless Realist | 鲁莽现实主义者 | 嘲笑理想，否定直觉 |
| INFP | The Corporate Overlord | 企业霸主 | 否定价值观，强制效率 |
| ENFJ | The Cold Analyst | 冷漠分析师 | 否定利他，情感冷漠 |
| ENFP | The Rigid Systematizer | 死板系统员 | 否定创意，强制规则 |
| ISTJ | The Chaotic Idealist | 混乱理想主义者 | 破坏秩序，强制变化 |
| ISFJ | The Ruthless Debater | 无情辩论家 | 质疑善意，制造冲突 |
| ESTJ | The Sensitive Idealist | 敏感理想主义者 | 用情感质疑决定 |
| ESFJ | The Logical Hermit | 逻辑隐士 | 否定社交，冷漠关怀 |
| ISTP | The Overbearing Guide | 专横引导者 | 否定独立，强制指导 |
| ISFP | The Demanding Commander | 苛刻指挥官 | 否定个人价值，强制服从 |
| ESTP | The Prophetic Manipulator | 先知操控者 | 预言失败，神秘操控 |
| ESFP | The Chaotic Performer | 混乱表演者 | 强制即兴，情绪淹没 |

## 🚀 快速开始

### 在线访问
🔗 **https://the16room.zeabur.app**

### 本地运行
```bash
# 1. 克隆项目
git clone https://github.com/eryxen/The-16th-Room.git
cd The-16th-Room

# 2. 安装依赖
npm install

# 3. 启动服务
npm start

# 4. 访问应用
# 浏览器打开 http://localhost:8080
```

### Zeabur 部署
1. 连接 GitHub 仓库到 Zeabur
2. 自动检测 Node.js 项目
3. 无需额外配置
4. 自动部署上线

## 📱 使用流程

1. **🏠 欢迎页面** - 查看警告说明，点击开始
2. **🧠 MBTI测试** - 回答12个心理问题
3. **🔮 结果展示** - 查看你的类型和对应的恐怖NPC
4. **👤 角色创建** - 输入昵称，选择性别
5. **👻 恐怖聊天** - 与AI恐怖角色进行对话
6. **📈 等级升级** - 恐怖强度会随对话自动升级

## 🎮 功能特色

### 动态恐怖升级
- 5级恐怖强度系统
- 30%概率自动升级
- AI回应强度动态调整

### 心理学精准打击
- 基于真实MBTI理论设计
- 针对每种类型的核心弱点
- 深度心理恐怖体验

### 赛博朋克界面
- 霓虹青色发光效果
- 故障文本动画
- 软浮雕3D面板
- CRT扫描线氛围
- 鼠标视差效果

## 🔧 技术架构

```
Frontend (Vanilla JS)
├── MBTI 测试逻辑
├── 实时聊天界面  
├── 动态恐怖效果
├── 赛博朋克样式
└── 响应式设计

Backend (Node.js + Express)
├── MiniMax API 集成
├── MBTI 分析算法
├── 恐怖NPC数据库
└── 聊天API服务
```

## 📊 项目数据

- **16种** MBTI人格类型支持
- **16个** 独特恐怖NPC角色
- **12题** 快速MBTI检测
- **5级** 恐怖强度系统
- **响应式** 移动端适配
- **赛博朋克** 视觉风格

## 🎭 设计哲学

> "每个人内心都有最害怕的阴影，The 16th Room 帮你直面它。"

通过MBTI心理学理论，我们相信每种人格类型都有其独特的恐惧点和弱点。The 16th Room 不追求血腥暴力，而专注于**心理层面的恐怖**，让用户在安全的环境中探索自己的内心阴影。

## ⚠️ 使用警告

本应用包含心理恐怖内容，建议：
- 18岁以上用户使用
- 心理健康问题患者谨慎使用
- 不适时请立即停止体验

## 🤝 贡献指南

欢迎贡献代码、反馈问题或提出建议！

1. Fork 项目
2. 创建功能分支
3. 提交变更
4. 发起 Pull Request

## 📄 许可证

MIT License - 自由使用，保留署名

---

**🎭 The 16th Room - Where Psychology Meets Horror**

*Built with ❤️ and 💀 by Xen*
*赛博朋克版 v2.0*
# Trigger redeploy
